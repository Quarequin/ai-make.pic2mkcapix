...
function setButtonState(state) {
	const states = {
		noImage: { run: true, copy: true, dl: true, text: "Convert Image" },
		imageLoaded: {
			run: false,
			copy: true,
			dl: true,
		...
		processing: { run: true, copy: true, dl: true, text: "Converting..." },
		done: { run: false, copy: false, dl: false, text: "Convert Image" },
	};
...
downloadButton.addEventListener("click", function (e) {
	...
	try {
		const dataUrl = canvas.toDataURL(
			originalMimeType === "image/gif" ? "image/png" : originalMimeType,
		);
